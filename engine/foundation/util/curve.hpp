...
namespace ImGui
{
	...
	static float CurveValueSmooth(float p, int maxpoints, const ImVec2 *points)
    {
    ...
    static int Curve(const char *label, const ImVec2& size, const int maxpoints, ImVec2 *points)
    {
        int modified = 0;
        int i;
        if (maxpoints < 2 || points == 0)
        ...
        if (hovered)
        {
            ...
            {
                modified = 1;
                ImVec2 pos = ImVec2((g.IO.MousePos.x - bb.Min.x) / (bb.Max.x - bb.Min.x), (g.IO.MousePos.y - bb.Min.y) / (bb.Max.y - bb.Min.y));
                pos.y = 1 - pos.y;              
                ...
                ImVec2 p = ImVec2(points[left].x - pos.x, points[left].y - pos.y);
                float p1d = sqrt(p.x*p.x + p.y*p.y);
                p = ImVec2(points[left+1].x - pos.x, points[left + 1].y - pos.y);
        ...
        for (i = 1; i < max; i++)
        {
            ...
            a.y = 1 - a.y;
            b.y = 1 - b.y;
            a = ImVec2(a.x * (bb.Max.x - bb.Min.x) + bb.Min.x, a.y * (bb.Max.y - bb.Min.y) + bb.Min.y);
			b = ImVec2(b.x * (bb.Max.x - bb.Min.x) + bb.Min.x, b.y * (bb.Max.y - bb.Min.y) + bb.Min.y);
        ...
        if (hovered)
        {
            ...
            for (i = 0; i < max; i++)
            {
                ImVec2 p = points[i];
                p.y = 1 - p.y;
                p = ImVec2(p.x * (bb.Max.x - bb.Min.x) + bb.Min.x, p.y * (bb.Max.y - bb.Min.y) + bb.Min.y);
                ImVec2 a = ImVec2(p.x - 2, p.y - 2);
                ImVec2 b = ImVec2(p.x + 2, p.y + 2);
