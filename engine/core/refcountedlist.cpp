...
RefCountedList::DumpLeaks()
{
    if (!this->IsEmpty())
    {
        SysFunc::DebugOut("\n\n\n******** VORTEX T REFCOUNTING LEAKS DETECTED:\n\n");
        ...
        for (iter = this->Begin(); iter != this->End(); iter++)
        {
			...
			{
				String msg;
				msg.Format("*** VORTEX T REFCOUNT LEAK: Object of class '%s' with debug identifier '%s' at address '0x%08lx', refcount is '%d'\n",
			...
			{
				String msg;
				msg.Format("*** VORTEX T REFCOUNT LEAK: Object of class '%s' at address '0x%08lx', refcount is '%d'\n",
					(*iter)->GetClassName().AsCharPtr(),
					(*iter),
					(*iter)->GetRefCount());
				SysFunc::DebugOut(msg.AsCharPtr());
        ...
        SysFunc::DebugOut("\n******** END OF VORTEX T REFCOUNT LEAK REPORT\n\n\n");
		this->refcountedDebugNames.Clear();
    }
    else
    {
        SysFunc::DebugOut("\n>>> HOORAY, NO VORTEX T REFCOUNT LEAKS!!!\n\n\n");
    }
}
...
{
	_assert(!this->refcountedDebugNames.Contains(ptr));
